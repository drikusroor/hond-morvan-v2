...
const CLOUDFLARE_RANGES = ["104.21.", "172.67.", "104.18.", "104.19.", "104.20."];
...
function printSummary(report: HealthReport) {
  ...
  if (report.dns) {
    ...
    if (report.dns.isGitHubPages) {
      ...
      console.log(`   ⚠️  ISSUE: Points to Cloudflare proxy instead of GitHub Pages`);
      console.log(`   💡 FIX: Disable Cloudflare proxy (orange → gray cloud) in DNS settings`);
    }
  }
  ...
  if (report.failed > 0) {
    ...
    for (const result of report.results.filter((r) => !r.ok)) {
      console.log(`   - ${result.url} (${result.status}): ${result.error || "Unknown error"}`);
    }
  }
